...
import type {
  CharacterApiSchema,
  LocationApi,
} from "@/types/server/character-api";
...
const parseCharacterLocation: (
  location: LocationApi,
  apiLocations: LocationApiSchema[]
) => Location | undefined = (location, apiLocations) => {
  const locationId = toLocation(location);
  if (!locationId) {
    return undefined;
  }
  const apiLocation = apiLocations.find(
...
const parseCharacterEpisodes: (
  episodes: string[],
  apiEpisodes: EpisodeApiSchema[]
) => Episode[] = (episodes, apiEpisodes) => {
